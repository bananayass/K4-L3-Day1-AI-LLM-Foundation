...
def call_openai(
    ...
    client= OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    start = time.perf_counter() # đo thời gian bat đầu
    ...
    end = time.perf_counter() # đo thời gian kết thúc của response
    latency = end - start # tính độ trễ của thời gian
    response_text = response.choices[0].message.content #lấy content
